...
void ParallelCompute::print_progress(
        uint32_t i, 
        uint32_t N, 
        std::optional<std::chrono::high_resolution_clock::time_point> run_start
) {
...
compute_result_t ParallelCompute::thread_compute(std::shared_ptr<Config> config, uint32_t num_threads) {
...
std::vector<compute_result_t> ParallelCompute::compute_serial(
...
std::vector<compute_result_t> ParallelCompute::compute_bspl(
    std::vector<std::shared_ptr<Config>> total_configs, 
    bool verbose
) {
    uint32_t total_runs = total_configs.size();
...
std::vector<compute_result_t> ParallelCompute::compute_omp(
    std::vector<std::shared_ptr<Config>> total_configs, 
    bool verbose
) {
    uint32_t total_runs = total_configs.size();
...
ParallelCompute::ParallelCompute(Params& metaparams, std::vector<std::shared_ptr<Config>> configs) : configs(configs) {
...
void ParallelCompute::compute(bool verbose) {
