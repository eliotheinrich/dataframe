...
double TimeConfig::correlation_coefficient(const std::vector<double> &y) {
    uint32_t n = y.size();
    ...
    double r = (sumxy - n*my*(n-1.)/2.)/((n-1.)*varx*vary);

    if (std::abs(r) > 1.0)
        throw std::runtime_error("Correlations cannot be greater than 1.0.");

    return r;
}

bool TimeConfig::samples_converged(const std::map<std::string, std::vector<Sample>> &samples) const {  
